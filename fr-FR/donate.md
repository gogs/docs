---
name: Donnation
---

# Donnation

<h4>
	<b><font color="red">Nous allons continuer à travailler dur avec votre énorme soutien !</font></b>
</h4>

<h4>
	<b>Donner par Paypal:</b>
	<p>
		<form action="https://www.paypal.com/cgi-bin/webscr" method="post" target="_top">
			<input type="hidden" name="cmd" value="_s-xclick">
			<input type="hidden" name="encrypted" value="-----BEGIN PKCS7----------END PKCS7-----
			">
			<input type="image" src="https://www.paypalobjects.com/en_US/i/btn/btn_donateCC_LG.gif" border="0" name="submit" alt="PayPal - The safer, easier way to pay online!">
			<img alt="" border="0" src="https://www.paypalobjects.com/en_US/i/scr/pixel.gif" width="1" height="1">
		</form>
	</p>
</h4>

### Votre don sera utilisé comme :

- Soutenir le développement de Beego
- Soutenir en maintenant les communautés
- Mettre à niveau le serveur
- Récompenser les grands contributeurs
- Tenir des activités et des conférences communautaires

### Listes de dons (dons en ordre de temps décroissant)

| Donated at       | Doneur    | Dons   | Commentaire               |
| ---------------- |:---------:| --------:| ----------------------- |
|  2015-2-22 | Mr. Rinke  | $20.00   |          |
|  2014-12-31 | Mr. Nakatsui  | $20.00   |          |
|  2014-10-13 | Mr. Tsang     | $50.00   |          |
|  2014-5-31 | Mr. Fanningert     | $50.00   |          |
